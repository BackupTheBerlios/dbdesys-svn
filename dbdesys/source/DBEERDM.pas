...
type
  ...
  TDMDBEER = class(TDataModule)
    ...
    function EERMySQLSyncDBScript(theModel: Pointer; DBConn: Pointer; Log: TStrings;
      KeepExTbls, StdInsertsOnCreate, StdInsertsSync: Boolean): string;
            ...
            begin
              ...
              begin
                SQLStr.Add('ALTER TABLE ' + theTable.ObjName + ' CHANGE COLUMN ' +
                  theColumn.PrevColName + ' ' + theTable.GetSQLColumnCreateDefCode(ColNr));
              ...
              //Just change type
                ...
                SQLStr.Add('ALTER TABLE ' + theTable.ObjName + ' MODIFY COLUMN ' +
                  theTable.GetSQLColumnCreateDefCode(ColNr));
            end;
          end;
          ...
          begin
            ...
            if (j = 0) then
              SQLStr.Add('ALTER TABLE ' + theTable.ObjName + ' ADD COLUMN ' +
                theTable.GetSQLColumnCreateDefCode(j) + ' FIRST')
            else
              SQLStr.Add('ALTER TABLE ' + theTable.ObjName + ' ADD COLUMN ' +
                theTable.GetSQLColumnCreateDefCode(j) + ' AFTER ' + TEERColumn(theTable.Columns[j - 1]).ColName);
          end;
        end;
...
function TDMDBEER.EERMySQLSyncDBScript(theModel: Pointer; DBConn: Pointer; Log: TStrings;
  KeepExTbls, StdInsertsOnCreate, StdInsertsSync: Boolean): string;
var EERModel: TEERModel;
  i, j, k, position,
    ColNr: integer;
...
begin
  ...
  //          DMDB.SchemaSQLQuery.SQL.Text:=
  //          DMDB.SchemaSQLQuery.ExecSQL;
            SQLScripts.Add(theTable.GetSQLCreateCode(True, //Define PKs
              True, //CreateIndices
              True, //DefineFK
              True, //TblOptions
              False));
          ...
          begin
            ...
            //Rename in DB
            SQLScripts.Add('RENAME TABLE ' + theTable.PrevTableName +
              ' TO ' + theTable.ObjName);
            {
            DMDB.SchemaSQLQuery.SQL.Text := 'RENAME TABLE ' + theTable.PrevTableName +
              ' TO ' + theTable.ObjName;
      ...
      begin
        ...
        begin
          ...
          SQLScripts.Add('drop table ' + DbTables[i]);
          {
          DMDB.SchemaSQLQuery.SQL.Text := 'drop table ' + DbTables[i];
          DMDB.SchemaSQLQuery.ExecSQL;
          }
    ...
    if (TableCreateCounter > 0) then
      if (TableCreateCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Tables created (SQL script).', 169,
          IntToStr(TableCreateCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Table created (SQL script).', 170));

    if (TableRenameCounter > 0) then
      if (TableRenameCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Tables renamed (SQL script).', 171,
          IntToStr(TableRenameCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Table renamed (SQL script).', 172));

    if (TableDropCounter > 0) then
      if (TableDropCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Tables dropped (SQL script).', 173,
          IntToStr(TableDropCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Table dropped (SQL script).', 174));

    if (ColumnAddCounter > 0) then
      if (ColumnAddCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Columns added (SQL script).', 175,
          IntToStr(ColumnAddCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Column added (SQL script).', 176));

    if (ColumnDelCounter > 0) then
      if (ColumnDelCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Columns deleted (SQL script).', 177,
          IntToStr(ColumnDelCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Column deleted (SQL script).', 178));

    if (ColumnModCounter > 0) then
      if (ColumnModCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Columns modified (SQL script).', 179,
          IntToStr(ColumnModCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Column modified (SQL script).', 180));

    if (IndexDropCounter > 0) then
      if (IndexDropCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Indices dropped (SQL script).', 181,
          IntToStr(IndexDropCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Index dropped (SQL script).', 182));

    if (IndexCreateCounter > 0) then
      if (IndexCreateCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Indices created (SQL script).', 183,
          IntToStr(IndexCreateCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Index created (SQL script).', 184));

    if (IndexUpdateCounter > 0) then
      if (IndexUpdateCounter > 1) then
        Log.Add(DMMain.GetTranslatedMessage('%s Indices updated (SQL script).', 185,
          IntToStr(IndexUpdateCounter)))
      else
        Log.Add(DMMain.GetTranslatedMessage('1 Index updated (SQL script).', 186));

    Log.Add('');

    Result := SQLScripts.Text;
    EERModel.ModelHasChanged;
  finally
    SQLScripts.Free;
    DbTables.Free;
