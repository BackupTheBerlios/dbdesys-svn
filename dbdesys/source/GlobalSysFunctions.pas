...
begin
{$IFDEF MSWINDOWS}
  SettingsPath := GetSpecialFolder(CSIDL_APPDATA) + PathDelim + 'DBDeSys' + PathDelim;

  //Create DBDeSys settings dir if it is not disabled by start parameter
  ...
  //Check if a DBDeSys Directory has been created in the Users
...
{$ELSE}
    //the settings are stored in a dir '.DBDeSys' in the users home dir
  SettingsPath := GetEnvironmentVariable('HOME') + PathDelim + '.DBDeSys' + PathDelim;
{$ENDIF}
