  ...
  begin
    ...
    begin
      ...
      try
        ...
        //When there is no DBDeSys table, create one when saving
        if (theTables.IndexOf('DBDeSys') = -1) then
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        DMDB.SchemaSQLQuery.SQL.Text := 'CREATE TABLE dbdesys (' +
      ...
      begin
        ...
        DMDB.SchemaSQLQuery.SQL.Text := 'SELECT name, idversion, version ' +
          'from dbdesys where idmodel=' + IntToStr(EERModel.IDModel) + ' ' +
      ...
      if (EERModel.IDModel = 0) then
      begin
        DMDB.SchemaSQLQuery.SQL.Text := 'SELECT max(idmodel)+1 as oldid FROM dbdesys';
        DMDB.SchemaSQLQuery.Open;
      ...
      begin
        DMDB.SchemaSQLQuery.SQL.Text := 'UPDATE dbdesys SET ' +
        ...
        DMDB.SchemaSQLQuery.SQL.Text := 'INSERT INTO dbdesys (' +
  ...
  begin
    ...
    begin
      ...
      DMDB.SchemaSQLQuery.SQL.Text :=
        'SELECT model, name FROM dbdesys WHERE ' +
        'idmodel=' +
...
begin
  if (SavedModelsTV.Selected <> nil) then
    ...
    begin
      ...
      DMDB.SchemaSQLQuery.SQL.Text := 'DELETE FROM dbdesys ' +
